...
void test_line(ei_surface_t surface, ei_rect_t* clipper)
{
	...
	pts[1].point.x = 600; pts[1].point.y = 400; pts[1].next = &pts[0];
...
void test_dot(ei_surface_t surface, ei_rect_t* clipper)
{
	...
	pts[1].point.x = 400; pts[1].point.y = 300; pts[1].next = &pts[0];
...
void test_polygon		(ei_surface_t			surface,
                                  ei_color_t			color,
                                  const ei_rect_t*		clipper)
{
        ...
        pts[0].point.x = 80; pts[0].point.y = 120; pts[0].next = &(pts[1]);
        pts[1].point.x = 360; pts[1].point.y = 40; pts[1].next = &(pts[2]);
        pts[2].point.x = 520; pts[2].point.y = 200; pts[2].next = &(pts[3]);
        pts[3].point.x = 480; pts[3].point.y = 400; pts[3].next = &(pts[4]);
        pts[4].point.x = 280; pts[4].point.y = 280; pts[4].next = &(pts[5]);
        pts[5].point.x = 80; pts[5].point.y = 360; pts[5].next = &(pts[0]);

        ei_draw_polygon(surface, &pts[0], color, clipper);
}
...
int main(int argc, char** argv)
{
	...
	ei_color_t      black       = {0x00, 0xff, 0x00, 0xff};
	...
	uint32_t* pixel_ptr = (uint32_t *) hw_surface_get_buffer(main_window);

	ei_rect_t		clipper		= ei_rect(ei_point(200, 150), ei_size(200, 300));
	clipper_ptr		= NULL;
	ei_fill		(main_window, &white, NULL);

	test_polygon(main_window, green, NULL);
	/* Draw polylines. */
